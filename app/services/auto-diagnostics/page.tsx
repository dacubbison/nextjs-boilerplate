...
export default function AutoDiagnostics() {
  ...
  const handleBookSubmit = (e: React.FormEvent) => {
    ...
    setErrorMessage('');
    const calendlyUrl = `https://calendly.com/david-toptechmobile/check-engine-light-diagnostic-100?a1=${encodeURIComponent(cleanedZip)}&a2=${encodeURIComponent(address)}&a3=${encodeURIComponent(year)}&a4=${encodeURIComponent(make)}&a5=${encodeURIComponent(model)}&a6=${encodeURIComponent(vin)}`;
  ...
  return (
    <main>
      <section className="service-hero">
        <h1>Mobile Auto Diagnostics and Check Engine Light Near Me in Kingwood TX</h1>
        <p>Advanced diagnostic services for check engine lights and vehicle issues in Montgomery County TX with fair and upfront pricing.</p>
        <Image src="/images/mobile-auto-diagnostics-woodlands.jpg" alt="Mobile auto diagnostics in Kingwood TX" width={800} height={400} loading="lazy" />
      </section>

      <section className="service-details">
        <h2>Our Diagnostics Services</h2>
        <p>Using state-of-the-art tools to diagnose check engine lights and other issues quickly and accurately.</p>
        <ul>
          <li>Advanced scanning for all makes</li>
          <li>Check engine light reset and repair</li>
          <li>Full vehicle health reports</li>
        </ul>
      </section>

      <section className="faqs">
        <h2>FAQs about Auto Diagnostics in Kingwood TX</h2>
        <details>
          <summary>What causes a check engine light?</summary>
          <p>Common issues like sensor failures or emissions problems—diagnosed with our mobile service.</p>
        </details>
        <details>
          <summary>How much is a diagnostic?</summary>
          <p>$100 with fair pricing; call for details.</p>
        </details>
      </section>
      ...
      {isBookModalOpen && (
        <div className="modal-overlay" aria-modal="true" role="dialog">
          <div className="book-modal-content">
            <div className="modal-header">Book Diagnostics</div>
            <form onSubmit={handleBookSubmit} className="book-form">
              ...
              <button type="submit" className="submit-btn">Submit & Book</button>
              <button type="button" onClick={toggleBookModal} className="close-btn">Close</button>
            </form>
          </div>
        </div>
      )}
      ...
      <Script type="application/ld+json">
        {JSON.stringify({
          ...
          "description": "Mobile auto diagnostics and check engine light services with fair pricing."
        })}
      </Script>
    </main>
  );
